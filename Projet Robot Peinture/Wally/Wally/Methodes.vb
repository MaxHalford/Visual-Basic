Module Methodes
    ...
    Function compterNoeuds(noeud As Node) As Integer
        ...
        Return value
    End Function
    Function getMaxDepth(ByVal noeud As Node, ByRef currentDepth As Integer) As Integer
        currentDepth += 1
    ...
    Sub assignDepth(ByVal noeud As Node)
        ...
        End If
    End Sub
    Sub treeToarray(ByVal noeud As Node, ByRef tableau As Node())
        If Not (noeud.successors Is Nothing) Then
            For i = 0 To noeud.successors.Length - 1
                ReDim Preserve tableau(tableau.Length)
                tableau(tableau.Length - 1) = noeud.successors(i)
                treeToarray(noeud.successors(i), tableau)
            Next
        End If
    End Sub
    Sub changeNode(ByRef newNode As Node, ByRef currentNode As Node, ByRef counter As Integer, ByVal numeroNoeud As Integer)
        If counter < numeroNoeud Then
            If Not (currentNode.successors Is Nothing) Then
                For i = 0 To currentNode.successors.Length - 1
                    counter += 1
                    changeNode(newNode, currentNode.successors(i), counter, numeroNoeud)
                Next
            End If
    ...
    Function croiser(ByVal sujets As Robot()) As Robot()
        ...
        Dim NouveauRobotUn As Robot = genererRobot() : NouveauRobotUn.rootNode = sujets(0).rootNode : NouveauRobotUn.treeAsString = sujets(0).treeAsString
        Dim NouveauRobotDeux As Robot = genererRobot() : NouveauRobotDeux.rootNode = sujets(1).rootNode : NouveauRobotDeux.treeAsString = sujets(1).treeAsString
        ...
        nouveauxRobots(0).capacite = getMaxDepth(nouveauxRobots(0).rootNode, nouveauxRobots(0).rootNode.profondeur) : nouveauxRobots(1).capacite = getMaxDepth(nouveauxRobots(1).rootNode, nouveauxRobots(1).rootNode.profondeur)
